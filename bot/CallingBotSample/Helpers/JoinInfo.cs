...
namespace CallingBotSample.Helpers
{
    ...
    {
        private static readonly Regex JoinUrlRegex = new Regex("https://teams\\.microsoft\\.com.*/(?<thread>[^/]+)/(?<message>[^/]+)\\?context=(?<context>{.*})");

        /// <summary>
        /// Try to find a Teams meeting join URL in a message.
        /// </summary>
        /// <param name="messageText">Message text sent to the bot.</param>
        /// <param name="joinUrl">Teams meeting join URL when present.</param>
        /// <returns>True when the message contains a Teams meeting join URL.</returns>
        public static bool TryExtractJoinUrl(string messageText, out string joinUrl)
        {
            joinUrl = null;
            ...
            var htmlDecodedText = WebUtility.HtmlDecode(messageText);
            var urlMatch = Regex.Match(htmlDecodedText, @"https://teams\.microsoft\.com/(l/meetup-join|meet)/[^\s<>""]+");
            ...
            joinUrl = urlMatch.Value.TrimEnd('.', ',', ';');
            return true;
        }

        /// <summary>
        /// Checks whether a link is the full Teams join link that includes chat and organizer context.
        /// </summary>
        /// <param name="joinURL">Teams meeting URL.</param>
        /// <returns>True when the URL can be parsed directly.</returns>
        public static bool IsFullJoinUrl(string joinURL)
        {
            ...
            return WebUtility.UrlDecode(joinURL).Contains("/l/meetup-join/", StringComparison.OrdinalIgnoreCase);
        }

        /// <summary>
        /// Parse Join URL into Teams chat information.
        /// </summary>
        /// <param name="joinURL">Join URL from Team's meeting body.</param>
        /// <returns>Parsed chat info.</returns>
        public static ChatInfo ParseChatInfo(string joinURL)
        {
            var match = MatchJoinUrl(joinURL);
        ...
        private static Match MatchJoinUrl(string joinURL)
        {
