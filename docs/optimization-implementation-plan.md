...
- Keep TTS timing predictable by keeping live TTS PCM at natural `1.0` after live testing exposed audible artifacts from backend PCM acceleration.
...
| Indonesian backend PCM speed | Updated | `TTS_BACKEND_SPEED_ID` was restored to natural `1.0` after live testing produced audible hiss/click artifacts from backend PCM resampling; Cartesia synthesis speed remains neutral. |
...
- Backend audio-quality follow-up disables Indonesian backend PCM speed-up as well, so live TTS output now avoids the custom streaming PCM resampler for both Chinese and Indonesian targets.
...
- TTS logs for Indonesian, Chinese, and English/default target output all show `backendPcmSpeed=1.0`.
