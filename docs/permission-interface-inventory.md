...
|---|---|---|---|---|---|
| `POST /api/auth/login` | Anonymous | Public | Account by username | Authentication failure uses existing auth error | Existing auth tests |
| `POST /api/auth/register` | Anonymous | Explicitly closed | None | `403` | `AuthControllerSecurityTest` |
| `/api/interpretation/public/**` | Anonymous | Public compatibility endpoints; active lookup and latency are rate limited | Public session or target user id | `400` invalid latency; `429` limited | `PublicInterpretationEndpointSecurityTest`, `AnonymousRequestRateLimiterTest` |
| `/api/interpretation/start`, `/stop`, `/status/**`, `/history/**`, `/results`, `/records/**`, `/session-speakers/**` | User | OWN only | `interpretation_session.user_id` | `401` no actor; `403` userId mismatch; `404` missing/other resource | `ResourceOwnershipPolicyTest`, `AuthContextTest` |
| `/api/interpretation/users/{userId}/sessions`, title/delete | User | SELF/OWN only | Actor user id, then session owner | `403` mismatch; `404` other session | `AuthContextTest`, `ResourceOwnershipPolicyTest` |
...
| `/api/pre-meeting/**` | User | SELF for usage/cross-meeting; OWN when meeting/session is supplied | Actor user id; meeting/session parent | `403` userId mismatch; `404` other meeting/session | `UserIdBoundaryControllerTest`, `ResourceOwnershipPolicyTest` |
| `/api/translate` | User | SELF only | Actor user id | `403` mismatch | `UserIdBoundaryControllerTest` |
| `/api/terminology/**`, `/api/asr-hotwords/**`, `/api/language-preferences/**`, `/api/audio-records/**`, `/api/cost/monthly-summary` | User | SELF only | Actor user id; service query also includes user id for child ids; missing owned terminology/hotword mutation is rejected | `403` mismatch; `404` other child id | `AuthContextTest`, `OwnedMutationSecurityTest` plus existing service tests |
...
|---|---|---|---|---|
| `/ws/asr` | User | Handshake token binds user id; `start` binds one owned session; audio/stop/translate require the same active binding; one controller connection per session | Handshake reject or policy close | `JwtHandshakeInterceptorTest`, `AsrWebSocketHandlerSecurityTest` |
| `/ws/share`, `/ws/share-audio` | Anonymous compatibility | Public until share capability migration | Residual risk accepted through P4 | Existing share smoke test required |
...
- `SystemUserInfo` role restrictions, dedicated `/api/teams-bot/**` service identity, and default-deny permission codes are P1/P4 work.
- Anonymous share paths remain public until share capability migration.
