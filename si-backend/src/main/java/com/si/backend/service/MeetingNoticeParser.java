...
public class MeetingNoticeParser {
    ...
    private static final Pattern PASSCODE_PATTERN = Pattern.compile(
            ...
            Pattern.CASE_INSENSITIVE);
    private static final Pattern TEAMS_URL_PATTERN = Pattern.compile(
            "(https?://[^\\s<>()\"']*(?:teams\\.microsoft\\.com|teams\\.live\\.com)[^\\s<>()\"']*)",
            Pattern.CASE_INSENSITIVE);
    ...
    public MeetingNoticeDetails parse(String text, String fallbackMeetingName) {
        ...
        MeetingNoticeDetails details = new MeetingNoticeDetails(
                ...
                meetingCode,
                passcode,
                parseMeetingUrl(normalized)
        );
        log.info("[MeetingNoticeParser] parse end, timeLineCount={}, hasVenue={}, hasMeetingCode={}, hasPasscode={}, hasMeetingUrl={}",
                timeLines.size(), !venue.isBlank(), !meetingCode.isBlank(), !passcode.isBlank(),
                !details.meetingUrl().isBlank());
        return details;
    }

    private String parseMeetingUrl(String text) {
        String url = findGroup(text, TEAMS_URL_PATTERN);
